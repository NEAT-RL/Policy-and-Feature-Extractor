...
env = normalize(normalize(GymEnv("MountainExtraLongCar-v0")))
...
policy.load_policy('policy_parameters/model-mountaincarlong.npz')
...
def evaluation(genomes, config):
    ...
    if generation % 20 == 0:
        with open('best_genomes/mountaincarlong-gen-{0}-fitness-{1}-genome'.format(generation, best_genome.fitness), 'wb') as f:
...
def run(config):
    ...
    with open('best_genomes/mountaincarlong-gen-winner-fitness-{0}-genome'.format(winner.fitness), 'wb') as f:
