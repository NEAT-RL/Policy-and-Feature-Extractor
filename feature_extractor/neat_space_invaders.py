...
def evaluation(genomes, config):
    ...
    if generation % 20 == 0:
        with open('best_genomes/space_invaders-gen-{0}-fitness-{1}-genome'.format(generation, best_genome.fitness), 'wb') as f:
...
def run(config):
    ...
    with open('best_genomes/space_invaders-gen-winner-fitness-{0}-genome'.format(winner.fitness), 'wb') as f:
